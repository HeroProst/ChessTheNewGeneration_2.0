...
public class RoomChoicerSceneManager : MonoBehaviourPunCallbacks
{
    ...
    public void JoinRoom()
    {
        ...
        PhotonNetwork.JoinRoom(pressedLobbyPreview.transform.parent.GetChild(0).GetComponent<TMP_Text>().text);
    }
    ...
    public override void OnRoomListUpdate(List<RoomInfo> roomList)
    {
        ...
        foreach (RoomInfo roomInfo in roomList)
        {
            ...
            if(lobbyNameOption == "" || roomInfo.Name.Contains(lobbyNameOption))
            {
                ...
                lobbyPreviewPanel.GetChild(0).GetComponent<TMP_Text>().text = roomInfo.Name;
                roomMapType = roomInfo.CustomProperties;
                lobbyPreviewPanel.GetChild(1).GetComponent<TMP_Text>().text = roomMapType["m"].ToString();
                lobbyPreviewPanel.GetChild(2).GetComponent<Button>().onClick.AddListener(JoinRoom);
            }
        }
    }
