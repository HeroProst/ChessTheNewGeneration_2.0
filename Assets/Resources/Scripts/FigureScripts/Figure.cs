...
public class Figure
{
	...
	public List<Cell> forbbidenCells = new List<Cell>();

	public List<Cell> allowedCells = new List<Cell>();
	...
	public static Figure.CollectionType GetCollectionTypeByName(string name)
    {
		switch (name)
		{
			...
			case "Extended":
				return Figure.CollectionType.Extended;
			case "None":
	...
	public static Figure.SpriteModel GetFigureSpriteByName(string name)
    {
		switch(name)
        {
			case "Origin":
				return SpriteModel.Origin;
			case "Alternative":
				return SpriteModel.Alternative;
			default:
				return SpriteModel.Origin;
		}
    }

	public static string GetStringNameByFigureSprite(Figure.SpriteModel spriteModel)
	{
		switch (spriteModel)
		{
			case SpriteModel.Origin:
				return "Origin";
			case SpriteModel.Alternative:
				return "Alternative";
			default:
				return "Origin";
		}
	...
	public int ReturnCountOfClones()
    {
		return cellsWithClone.Count;
    }
	
	public void DeleteAllClones()
	{
		foreach (Cell c in cellsWithClone)
			if(c.CurrentFigure.FigureSide == FigureSide)
	...
	public void DeleteAllCloneExeptSelf()
	{
		...
		{
			if(cellsWithClone[i].CurrentFigure.FigureSide == FigureSide)
