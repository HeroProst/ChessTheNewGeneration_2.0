	...
	public List<Cell> typeMove(GameField gameField, Figure currentFigure)
	{
		...
		for(int i = 0; i < 5;i++)
		{
			if(i % 2 != 0)
			{
				if (gameField.FindCellByCoordinates(currentFigure.YPos - 2, currentFigure.XPos + i - 2) != null)
				{
					if (currentFigure.forbbidenCells.Contains(gameField.FindCellByCoordinates(currentFigure.YPos - 2, currentFigure.XPos + i - 2)))
						continue;
					if (currentFigure.allowedCells.Count > 0)
					{
						if (!currentFigure.allowedCells.Contains(gameField.FindCellByCoordinates(currentFigure.YPos - 2, currentFigure.XPos + i - 2)))
							continue;
					}
					if (gameField.FindCellByCoordinates(currentFigure.YPos - 2, currentFigure.XPos + i - 2).CurrentFigure == null)
		...
		for (int i = 0; i < 5; i++)
		{
			if (i % 2 != 0)
			{
				if (gameField.FindCellByCoordinates(currentFigure.YPos + i - 2, currentFigure.XPos - 2) != null)
				{
					if (currentFigure.forbbidenCells.Contains(gameField.FindCellByCoordinates(currentFigure.YPos + i - 2, currentFigure.XPos - 2)))
						continue;
					if (currentFigure.allowedCells.Count > 0)
					{
						if (!currentFigure.allowedCells.Contains(gameField.FindCellByCoordinates(currentFigure.YPos + i - 2, currentFigure.XPos - 2)))
							continue;
					}
					if (gameField.FindCellByCoordinates(currentFigure.YPos + i - 2, currentFigure.XPos - 2).CurrentFigure == null)
