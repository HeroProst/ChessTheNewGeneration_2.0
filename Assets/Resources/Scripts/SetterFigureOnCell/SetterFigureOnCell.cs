...
public class SetterFigureOnCell : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ISetFactory figureFactory = ISetFactory.CreateSetFactory(Figure.CollectionType.Default);
        Transform[] rowsInGameBoarder = new Transform[gameObject.transform.childCount];
        for (int i = 0; i < gameObject.transform.childCount; i++)
        {
            rowsInGameBoarder[i] = gameObject.transform.GetChild(i);
        }
        for (int i = 0; i < rowsInGameBoarder.Length; i++)
        {
            for (int j = 0; j < rowsInGameBoarder[i].childCount; j++)
            {
                BoardCellContainer cell = rowsInGameBoarder[i].GetChild(j).gameObject.GetComponent<BoardCellContainer>();
                if (cell.figureCollection == Figure.CollectionType.None || cell.figureType == Figure.TypesOfFigure.None)
                    continue;
                figureFactory = ISetFactory.CreateSetFactory(cell.figureCollection);
                cell.price = figureFactory.CreateFigure(cell.figureType, Figure.Side.Bottom, Figure.SpriteModel.Origin).price;
                SetCurrantCurancy(cell.price);
            }
        }
    }
